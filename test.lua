...
function RayfieldLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = "Cracked Hub"
	LoadingFrame.Subtitle.Text = "ㄱㅇㅇ이/가 크랙했습니다"
	...
	if (Settings.KeySystem) then
		...
		if typeof(Settings.KeySettings.Key) == "string" then Settings.KeySettings.Key = {"ㄱㅇㅇ"} end

		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				local Success, Response = pcall(function()
					Settings.KeySettings.Key[i] = string.gsub("ㄱㅇㅇ", " ", "")
				end)
				if not Success then
		...
		if not Passthrough then
			...
			KeyMain.Title.Text = "Cracked Hub"
			KeyMain.Subtitle.Text = "ㄱㅇㅇ가 크랙함"
			KeyMain.NoteMessage.Text = "키: ㄱㅇㅇ"
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				local FoundKey = ''
        if KeyMain.Input.InputBox.Text == "ㄱㅇㅇ" then
          KeyFound = true
          FoundKey = "ㄱㅇㅇ"
        end
				if KeyFound then 
